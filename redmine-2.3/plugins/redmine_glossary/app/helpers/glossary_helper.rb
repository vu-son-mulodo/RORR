...
module GlossaryHelper
  ...
  def updated_by(updated, author)
    ...
    author_tag = (author.is_a?(User) && !author.is_a?(AnonymousUser)) ? link_to(h(author), :controller => 'users', :action => 'show', :id => author) : h(author || 'Anonymous')
    l(:label_updated_time_by, :author => author_tag, :age => time_tag)
  end

  def prompt_to_remote(name, text, param, url, html_options = {})
    html_options[:onclick] = "promptToRemote('#{text}', '#{param}', '#{url_for(url)}', 'authenticity_token=#{form_authenticity_token}'); return false;"
    link_to name, {:remote => true}, html_options
  end

  def render_option_categories(list)
    temp = '<option></option>'
    list.each {|item|
